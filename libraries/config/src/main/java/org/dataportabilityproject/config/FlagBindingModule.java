...
public class FlagBindingModule extends AbstractModule {
  ...
  private void bindFlags() {
    // Automatically bind all flags in settingsExtension to a Named annotation. e.g., binds:
    // settingsExtension.cloud() to a String annotated with @Named("cloud"), so core classes
    // may inject '@Named("cloud") String cloud'.
    for (Method method : settingsExtension.getClass().getMethods()) {
      ...
      Object flagValue;
      try {
        flagValue = method.invoke(settingsExtension);
