...
public class WorkerMain {
  ...
  public void initialize() {
    WorkerSettingsExtension settingsExtension = getSettingsExtension();
    settingsExtension.initialize(null);
    ExtensionContext context = new WorkerExtensionContext(settingsExtension);
    ...
    ServiceLoader.load(ServiceExtension.class)
        ...
        .forEachRemaining(serviceExtension ->
            serviceExtension.initialize(context));
    ...
    Injector injector =
        Guice.createInjector(new WorkerModule(context, cloudExtension, transferExtensions,
                symmetricKeyGenerator, asymmetricKeyGenerator, settingsExtension));
  ...
  private static WorkerSettingsExtension getSettingsExtension() {
    ImmutableList.Builder<WorkerSettingsExtension> extensionsBuilder = ImmutableList.builder();
    ServiceLoader.load(WorkerSettingsExtension.class).iterator()
        .forEachRemaining(extensionsBuilder::add);
    ImmutableList<WorkerSettingsExtension> extensions = extensionsBuilder.build();
