        ...
        for (TransferExtension extension : extensions) {
            if (extension.getServiceId().equals(exportService)) {
                ...
                Preconditions.checkState(exporter == null,
                    "Can only instantiate exporter from one matching "
                        + "TransferExtension. Found existing one already: " + exporter.getClass());
                exporter = extension.getExporter(transferDataType);
            } else if (extension.getServiceId().equals(importService)) {
                extension.initialize(extensionContext);
                Preconditions.checkState(importer == null,
                    "Can only instantiate importer from one matching "
                        + "TransferExtension. Found existing one already: " + importer.getClass());
