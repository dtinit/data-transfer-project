...
public abstract class JobStoreWithValidator implements JobStore {
    ...
    public void claimJob(UUID jobId, PortabilityJob job) throws IOException {
        updateJob(jobId, job, (previous, updated) -> Preconditions.checkState(previous.jobAuthorization().state() == JobAuthorization.State.CREDS_AVAILABLE));
    }
    ...
    private static void validateForUpdateStateToCredsAvailable(PortabilityJob job) {
        ...
        Preconditions.checkArgument(dataType != null, "Missing valid dataTypeParam: %s", dataType);
    ...
    public void updateJobWithCredentials(UUID jobId, PortabilityJob job) throws IOException {
        updateJob(jobId, job, ((previous, updated) -> {
            Preconditions.checkState(previous.jobAuthorization().state() == CREDS_ENCRYPTION_KEY_GENERATED);
     ...
     */
    protected abstract void updateJob(UUID jobId, PortabilityJob job, JobUpdateValidator validator) throws IOException;
