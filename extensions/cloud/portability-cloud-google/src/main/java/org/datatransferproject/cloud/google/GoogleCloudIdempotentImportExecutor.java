...
public class GoogleCloudIdempotentImportExecutor implements IdempotentImportExecutor {
    ...
    private <T extends Serializable> void addResult(String idempotentId, T result) throws IOException {
    ...
    @Override
    public <T extends Serializable> T getCachedValue(String idempotentId) throws IllegalArgumentException {
        if (!knownValues.containsKey(idempotentId)) {
    ...
    private Map<String, Serializable> getKnownValuesForJob(UUID jobId) {
        ...
        Query<Entity> query = Query.newEntityQueryBuilder().setKind(IDEMPOTENT_RESULTS_KIND).setFilter(CompositeFilter.and(PropertyFilter.eq(JOB_ID_FIELD, String.valueOf(jobId)))).build();
        QueryResults<Entity> results = datastore.run(query);
        while (results.hasNext()) {
            Entity result = results.next();
            dataStoreKnownValues.put(result.getString(IDEMPOTENT_ID_FIELD), result.getString(RESULTS_FIELD));
        }
    ...
    private <T extends Serializable> Entity createResultEntity(String idempotentId, T result) throws IOException {
        return createResultEntity(idempotentId, this.jobId, result);
    }

    @VisibleForTesting
    <T extends Serializable> Entity createResultEntity(String idempotentId, UUID jobId, T result) throws IOException {
        return GoogleCloudUtils.createEntityBuilder(getResultsKey(idempotentId, jobId), ImmutableMap.of(RESULTS_FIELD, result, JOB_ID_FIELD, jobId.toString(), IDEMPOTENT_ID_FIELD, idempotentId)).build();
    }

    private Key getResultsKey(String idempotentId, UUID jobId) {
        return datastore.newKeyFactory().setKind(IDEMPOTENT_RESULTS_KIND).newKey(jobId + "_" + idempotentId);
    }
    ...
    Entity createErrorEntity(String idempotentId, UUID jobId, ErrorDetail error) throws IOException {
        return GoogleCloudUtils.createEntityBuilder(getErrorKey(idempotentId, jobId), ImmutableMap.of(ERROR_FIELD, objectMapper.writeValueAsString(error), JOB_ID_FIELD, jobId.toString(), IDEMPOTENT_ID_FIELD, idempotentId)).build();
    }

    private Key getErrorKey(String idempotentId, UUID jobId) {
        return datastore.newKeyFactory().setKind(IDEMPONTENT_ERRORS_KIND).newKey(jobId + "_" + idempotentId);
    }
}
