...
class GoogleDtpInternalMetricRecorder implements DtpInternalMetricRecorder {
    ...
    private GoogleDtpInternalMetricRecorder(GoogleCredentials credentials, String projectId) throws IOException {
        // Enable OpenCensus exporters to export metrics to Stackdriver Monitoring.
        // Exporters use Application Default Credentials to authenticate.
        // See https://developers.google.com/identity/protocols/application-default-credentials
        // for more details.
        StackdriverStatsConfiguration configuration = StackdriverStatsConfiguration.builder().setCredentials(credentials).setProjectId(projectId).setExportInterval(io.opencensus.common.Duration.create(EXPORT_INTERVAL_SECONDS, 0)).build();
    ...
    private void setupView(Measure measure, TagKey... keys) {
        ...
        View view = View.create(View.Name.create(NAME_BASE + measure.getName()), measure.getDescription(), measure, Aggregation.Count.create(), ImmutableList.copyOf(keys));
        viewManager.registerView(view);
    }

    @Override
    public void startedJob(DataVertical dataType, String exportService, String importService) {
        TagContext tctx = tagger.emptyBuilder().put(KEY_DATA_TYPE, TagValue.create(dataType.getDataType()), TAG_METADATA).put(KEY_EXPORT_SERVICE, TagValue.create(exportService), TAG_METADATA).put(KEY_IMPORT_SERVICE, TagValue.create(importService), TAG_METADATA).build();
    ...
    public void exportPageAttemptFinished(DataVertical dataType, String service, boolean success, Duration duration) {
        TagContext tctx = tagger.emptyBuilder().put(KEY_DATA_TYPE, TagValue.create(dataType.getDataType()), TAG_METADATA).put(KEY_EXPORT_SERVICE, TagValue.create(service), TAG_METADATA).put(KEY_SUCCESS, TagValue.create(Boolean.toString(success)), TAG_METADATA).build();
        try (Scope ss = tagger.withTagContext(tctx)) {
            STATS_RECORDER.newMeasureMap().put(EXPORT_PAGE_ATTEMPT, 1).put(EXPORT_PAGE_ATTEMPT_DURATION, duration.toMillis()).record();
        }
    }
    ...
    public void exportPageFinished(DataVertical dataType, String service, boolean success, Duration duration) {
        TagContext tctx = tagger.emptyBuilder().put(KEY_DATA_TYPE, TagValue.create(dataType.getDataType()), TAG_METADATA).put(KEY_EXPORT_SERVICE, TagValue.create(service), TAG_METADATA).put(KEY_SUCCESS, TagValue.create(Boolean.toString(success)), TAG_METADATA).build();
        try (Scope ss = tagger.withTagContext(tctx)) {
            STATS_RECORDER.newMeasureMap().put(EXPORT_PAGE, 1).put(EXPORT_PAGE_DURATION, duration.toMillis()).record();
        }
    }

    @Override
    public void importPageAttemptFinished(DataVertical dataType, String service, boolean success, Duration duration) {
        TagContext tctx = tagger.emptyBuilder().put(KEY_DATA_TYPE, TagValue.create(dataType.getDataType()), TAG_METADATA).put(KEY_IMPORT_SERVICE, TagValue.create(service), TAG_METADATA).put(KEY_SUCCESS, TagValue.create(Boolean.toString(success)), TAG_METADATA).build();
        try (Scope ss = tagger.withTagContext(tctx)) {
            STATS_RECORDER.newMeasureMap().put(IMPORT_PAGE_ATTEMPT, 1).put(IMPORT_PAGE_ATTEMPT_DURATION, duration.toMillis()).record();
        }
    }
    ...
    public void importPageFinished(DataVertical dataType, String service, boolean success, Duration duration) {
        TagContext tctx = tagger.emptyBuilder().put(KEY_DATA_TYPE, TagValue.create(dataType.getDataType()), TAG_METADATA).put(KEY_IMPORT_SERVICE, TagValue.create(service), TAG_METADATA).put(KEY_SUCCESS, TagValue.create(Boolean.toString(success)), TAG_METADATA).build();
        try (Scope ss = tagger.withTagContext(tctx)) {
            STATS_RECORDER.newMeasureMap().put(IMPORT_PAGE, 1).put(IMPORT_PAGE_DURATION, duration.toMillis()).record();
        }
    }
    ...
    public void finishedJob(DataVertical dataType, String exportService, String importService, boolean success, Duration duration) {
        TagContext tctx = tagger.emptyBuilder().put(KEY_DATA_TYPE, TagValue.create(dataType.getDataType()), TAG_METADATA).put(KEY_EXPORT_SERVICE, TagValue.create(exportService), TAG_METADATA).put(KEY_IMPORT_SERVICE, TagValue.create(importService), TAG_METADATA).put(KEY_SUCCESS, TagValue.create(Boolean.toString(success)), TAG_METADATA).build();
        try (Scope ss = tagger.withTagContext(tctx)) {
            STATS_RECORDER.newMeasureMap().put(JOB_FINISHED, 1).put(JOB_FINISHED_DURATION, duration.toMillis()).record();
        }
    }
    ...
    public void recordGenericMetric(DataVertical dataType, String service, String tag, boolean bool) {
        TagContext tctx = tagger.emptyBuilder().put(KEY_DATA_TYPE, TagValue.create(dataType.getDataType()), TAG_METADATA).put(KEY_GENERIC_SERVICE, TagValue.create(service), TAG_METADATA).put(KEY_GENERIC_TAG, TagValue.create(tag), TAG_METADATA).put(KEY_GENERIC_BOOL, TagValue.create(Boolean.toString(bool)), TAG_METADATA).build();
        try (Scope ss = tagger.withTagContext(tctx)) {
            STATS_RECORDER.newMeasureMap().put(GENERIC_BOOLEAN, 1).record();
        }
    }

    @Override
    public void recordGenericMetric(DataVertical dataType, String service, String tag, Duration duration) {
        TagContext tctx = tagger.emptyBuilder().put(KEY_DATA_TYPE, TagValue.create(dataType.getDataType()), TAG_METADATA).put(KEY_GENERIC_SERVICE, TagValue.create(service), TAG_METADATA).put(KEY_GENERIC_TAG, TagValue.create(tag), TAG_METADATA).build();
        try (Scope ss = tagger.withTagContext(tctx)) {
    ...
    public void recordGenericMetric(DataVertical dataType, String service, String tag, int value) {
        TagContext tctx = tagger.emptyBuilder().put(KEY_DATA_TYPE, TagValue.create(dataType.getDataType()), TAG_METADATA).put(KEY_GENERIC_SERVICE, TagValue.create(service), TAG_METADATA).put(KEY_GENERIC_TAG, TagValue.create(tag), TAG_METADATA).build();
        try (Scope ss = tagger.withTagContext(tctx)) {
            STATS_RECORDER.newMeasureMap().put(GENERIC_COUNT, value).record();
        }
    }
}
