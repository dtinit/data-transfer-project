...
class GoogleDtpInternalMetricRecorder implements DtpInternalMetricRecorder {
  ...
  private static final Measure.MeasureLong JOB_STARTED =
      Measure.MeasureLong.create("job_start", "Number of jobs that were started", "count");

  private static final Measure.MeasureLong JOB_FINISHED =
      Measure.MeasureLong.create("job_finished", "Number of jobs that finished", "count");

  private static final Measure.MeasureLong JOB_FINISHED_DURATION =
      Measure.MeasureLong.create("job_finished_duration", "Duration of a job in MS", "ms");

  private static final Measure.MeasureLong EXPORT_PAGE_ATTEMPT =
      Measure.MeasureLong.create("export_page_attempt", "A single export attempt", "count");

  private static final Measure.MeasureLong EXPORT_PAGE_ATTEMPT_DURATION =
      Measure.MeasureLong.create(
          "export_page_attempt_duration", "Duration of a single export attempt in MS", "ms");

  private static final Measure.MeasureLong EXPORT_PAGE =
      Measure.MeasureLong.create("export_page", "An export attempt including all retries", "count");

  private static final Measure.MeasureLong EXPORT_PAGE_DURATION =
      Measure.MeasureLong.create(
          "export_page_duration", "Duration of an export page including retries in MS", "ms");

  private static final Measure.MeasureLong IMPORT_PAGE_ATTEMPT =
      Measure.MeasureLong.create("import_page_attempt", "A single import attempt", "count");

  private static final Measure.MeasureLong IMPORT_PAGE_ATTEMPT_DURATION =
      Measure.MeasureLong.create(
          "import_page_attempt_duration", "Duration of a single import attempt in MS", "ms");

  private static final Measure.MeasureLong IMPORT_PAGE =
      Measure.MeasureLong.create("import_page", "An import attempt including all retries", "count");

  private static final Measure.MeasureLong IMPORT_PAGE_DURATION =
      Measure.MeasureLong.create(
          "import_page_duration", "Duration of an import page including retries in MS", "ms");

  private static final Measure.MeasureLong GENERIC_COUNT =
      Measure.MeasureLong.create(
          "generic_count",
          "A generic counter that services can use to hold arbitrary metrics",
          "count");

  private static final Measure.MeasureLong GENERIC_DURATION =
      Measure.MeasureLong.create(
          "generic_duration",
          "A generic counter that services can use to hold arbitrary duration metrics",
          "ms");

  private static final Measure.MeasureLong GENERIC_BOOLEAN =
      Measure.MeasureLong.create(
          "generic_boolean",
  ...
  static synchronized GoogleDtpInternalMetricRecorder getInstance() throws IOException {
    if (INSTANCE == null) {
      INSTANCE =
          new GoogleDtpInternalMetricRecorder(
              GoogleCredentials.getApplicationDefault(), GoogleCloudExtensionModule.getProjectId());
    }
  ...
  private void setupViews() {
    ...
    setupView(
        JOB_FINISHED_DURATION, KEY_DATA_TYPE, KEY_EXPORT_SERVICE, KEY_IMPORT_SERVICE, KEY_SUCCESS);
    ...
    setupView(
        GENERIC_BOOLEAN, KEY_DATA_TYPE, KEY_GENERIC_SERVICE, KEY_GENERIC_TAG, KEY_GENERIC_BOOL);
