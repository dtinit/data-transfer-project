...
public class StreamingContentClient {
     ...
     * @throws IOException
     */
    public StreamingContentClient(@NotNull final String url, @NotNull final StreamingMode mode, @NotNull Monitor monitor) throws IOException {
        ...
        try {
            ...
            if (connection.getResponseCode() != HttpStatus.SC_OK) {
                if (connection.getResponseCode() == HttpStatus.SC_MOVED_PERMANENTLY) {
                    ...
                    if (connection.getResponseCode() != HttpStatus.SC_OK) {
                        throw new IOException("Error response code when trying to download content " + connection.getResponseCode());
                    }
                } else {
                    throw new IOException("Error response code when trying to download content " + connection.getResponseCode());
                }
