...
public class RoundTripToRdfTest {
    ...
    private void checkEquality(VCard input, VCard output) {
        ...
        assertWithMessage("Structured names match").that(output.getStructuredName()).isEqualTo(input.getStructuredName());
        assertWithMessage("Formatted names match for %s", name).that(output.getFormattedName()).isEqualTo(input.getFormattedName());
        assertWithMessage("telephone numbers match for %s", name).that(output.getTelephoneNumbers()).containsAllIn(input.getTelephoneNumbers());
