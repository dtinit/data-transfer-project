...
public class SolidContactsImport implements Importer<CookiesAndUrlAuthData, ContactsModelWrapper> {
    ...
    private static final ImmutableList<Resource> EMAIL_TYPE_RESOURCES = ImmutableList.of(VCARD4.Home, VCARD4.Work, // Deprecated Types:
    VCARD4.Dom, VCARD4.Internet, VCARD4.ISDN, VCARD4.Pref);
    ...
    static {
        MAP_OF_EMAIL_TYPES = ImmutableMap.copyOf(EMAIL_TYPE_RESOURCES.stream().collect(Collectors.toMap(r -> r.getLocalName().toLowerCase(), Function.identity())));
        MAP_OF_PHONE_TYPES = ImmutableMap.copyOf(PHONE_TYPE_RESOURCES.stream().collect(Collectors.toMap(r -> r.getLocalName().toLowerCase(), Function.identity())));
    }
    ...
    private void createContent(IdempotentImportExecutor idempotentExecutor, String baseUrl, List<VCard> people, SolidUtilities utilities) throws Exception {
        ...
        idempotentExecutor.executeOrThrowException(baseUrl + containerUrl, addressBookSlug, () -> createIndex(baseUrl + containerUrl, addressBookSlug, utilities));
        String personDirectory = idempotentExecutor.executeOrThrowException(baseUrl + containerUrl + "person", addressBookSlug, () -> createPersonDirectory(baseUrl + containerUrl, utilities));
        Map<String, VCard> insertedPeople = new HashMap<>();
        for (VCard person : people) {
            insertedPeople.put(importPerson(idempotentExecutor, person, baseUrl, personDirectory, utilities), person);
        }
        // people.stream()
        //     .collect(Collectors.toMap(
        //         p -> importPerson(idempotentExecutor, p, baseUrl, personDirectory, utilities),
        //         Function.identity()));
        idempotentExecutor.executeOrThrowException("peopleFile", addressBookSlug, () -> createPeopleFile(baseUrl, containerUrl, insertedPeople, utilities));
    }

    private String importPerson(IdempotentImportExecutor executor, VCard person, String baseUrl, String personDirectory, SolidUtilities utilities) throws Exception {
        return executor.executeAndSwallowIOExceptions(Integer.toString(person.hashCode()), person.getFormattedName().getValue(), () -> insertPerson(baseUrl, personDirectory, person, utilities));
    }
    ...
    private String createIndex(String url, String slug, SolidUtilities utilities) throws Exception {
        ...
        containerResource.addProperty(model.createProperty(VCARD4.NS + "nameEmailIndex"), model.createResource("people.ttl"));
        containerResource.addProperty(model.createProperty(VCARD4.NS + "groupIndex"), model.createResource("groups.ttl"));
    ...
    private String insertPerson(String baseUrl, String container, VCard person, SolidUtilities utilities) {
        ...
        try {
            String directory = utilities.postContent(baseUrl + container, null, BASIC_CONTAINER_TYPE, personContainerModel);
            return utilities.postContent(baseUrl + directory, "index", BASIC_RESOURCE_TYPE, getPersonModel(person));
        } catch (IOException e) {
            throw new IllegalStateException("Couldn't insert: " + person.getFormattedName() + " into: " + baseUrl + container, e);
        }
    }

    private String createPeopleFile(String baseUrl, String containerUrl, Map<String, VCard> importedPeople, SolidUtilities utilities) throws Exception {
        ...
        for (String insertedId : importedPeople.keySet()) {
            ...
            personResource.addProperty(peopleModel.createProperty(VCARD4.NS, "inAddressBook"), indexResource);
        }
        return utilities.postContent(baseUrl + containerUrl, "people", BASIC_RESOURCE_TYPE, peopleModel);
    }
    ...
    final static Model getPersonModel(VCard vcard) {
        ...
        for (StructuredName structuredName : vcard.getStructuredNames()) {
            ...
            structuredName.getPrefixes().forEach(prefix -> strucName.addProperty(VCARD4.hasHonorificPrefix, prefix));
            structuredName.getSuffixes().forEach(suffix -> strucName.addProperty(VCARD4.hasHonorificSuffix, suffix));
            structuredName.getAdditionalNames().forEach(additional -> strucName.addProperty(VCARD4.hasAdditionalName, additional));
        ...
        for (Email email : vcard.getEmails()) {
            ...
            if (email.getTypes().isEmpty()) {
                ...
                for (EmailType type : email.getTypes()) {
                    for (Resource emailTypeResource : getPhoneOrMailTypes(type.getValue(), MAP_OF_EMAIL_TYPES)) {
     ...
     */
    private static ImmutableList<Resource> getPhoneOrMailTypes(String type, Map<String, Resource> map) {
        return ImmutableList.copyOf(Arrays.stream(type.split(",")).map(t -> {
