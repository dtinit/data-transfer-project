...
public class MicrosoftMediaExporterTest {
    ...
    public void setUp() throws IOException {
        ...
        microsoftMediaExporter = new MicrosoftMediaExporter(credentialFactory, new JacksonFactory(), mediaInterface, monitor);
        when(mediaInterface.getDriveItems(any(Optional.class), any(Optional.class))).thenReturn(driveItemsResponse);
        when(mediaInterface.getDriveItemsFromSpecialFolder(any(MicrosoftSpecialFolder.FolderType.class))).thenReturn(driveItemsResponse);
    ...
    public void exportOneAlbumWithNextPage() throws IOException {
        ...
        // Run
        ExportResult<MediaContainerResource> result = microsoftMediaExporter.exportOneDrivePhotos(null, Optional.empty(), Optional.empty(), uuid);
        // Verify method calls
        verify(mediaInterface).getDriveItemsFromSpecialFolder(MicrosoftSpecialFolder.FolderType.photos);
        ...
        assertThat(actualAlbums.stream().map(MediaAlbum::getId).collect(Collectors.toList())).containsExactly(FOLDER_ID);
    ...
    public void exportAlbumWithoutNextPage() throws IOException {
        ...
        // Run
        ExportResult<MediaContainerResource> result = microsoftMediaExporter.exportOneDrivePhotos(null, Optional.empty(), Optional.of(inputPaginationToken), uuid);
        // Verify method calls
    ...
    public void exportMediaWithNextPage() throws IOException {
        // Setup
        when(driveItemsResponse.getNextPageLink()).thenReturn(null);
        when(driveItemsResponse.getDriveItems()).thenReturn(new MicrosoftDriveItem[] { setUpSinglePhoto(PHOTO_FILENAME, PHOTO_URI, PHOTO_ID), setUpSingleVideo(VIDEO_FILENAME, VIDEO_URI, VIDEO_ID) });
        when(driveItemsResponse.getNextPageLink()).thenReturn(DRIVE_PAGE_URL);
        IdOnlyContainerResource idOnlyContainerResource = new IdOnlyContainerResource(FOLDER_ID);
        // Run
        ExportResult<MediaContainerResource> result = microsoftMediaExporter.exportOneDrivePhotos(null, Optional.of(idOnlyContainerResource), Optional.empty(), uuid);
        // Verify method calls
        ...
        assertThat(actualPhotos.stream().map(PhotoModel::getFetchableUrl).collect(Collectors.toList())).containsExactly(PHOTO_URI);
        assertThat(actualPhotos.stream().map(PhotoModel::getAlbumId).collect(Collectors.toList())).containsExactly(FOLDER_ID);
        assertThat(actualPhotos.stream().map(PhotoModel::getTitle).collect(Collectors.toList())).containsExactly(PHOTO_FILENAME);
        ...
        assertThat(actualVideos.stream().map(VideoModel::getFetchableUrl).collect(Collectors.toList())).containsExactly(VIDEO_URI);
        assertThat(actualVideos.stream().map(VideoModel::getAlbumId).collect(Collectors.toList())).containsExactly(FOLDER_ID);
        assertThat(actualVideos.stream().map(VideoModel::getName).collect(Collectors.toList())).containsExactly(VIDEO_FILENAME);
    ...
    public void exportMediaWithoutNextPage() throws IOException {
        ...
        IdOnlyContainerResource idOnlyContainerResource = new IdOnlyContainerResource(FOLDER_ID);
        // Run
        ExportResult<MediaContainerResource> result = microsoftMediaExporter.exportOneDrivePhotos(null, Optional.of(idOnlyContainerResource), Optional.of(inputPaginationToken), uuid);
        // Verify method calls
    ...
    public void exportAlbumAndMediaWithNextPage() throws IOException {
        // Setup
        when(driveItemsResponse.getDriveItems()).thenReturn(new MicrosoftDriveItem[] { setUpSingleAlbum(), setUpSinglePhoto(PHOTO_FILENAME, PHOTO_URI, PHOTO_ID) });
        when(driveItemsResponse.getNextPageLink()).thenReturn(DRIVE_PAGE_URL);
        // Run
        ExportResult<MediaContainerResource> result = microsoftMediaExporter.exportOneDrivePhotos(null, Optional.empty(), Optional.empty(), uuid);
        // Verify method calls
