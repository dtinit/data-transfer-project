...
public class MicrosoftCalendarImporter implements Importer<TokenAuthData, CalendarContainerResource> {
    private static final String CALENDAR_URL = "me/calendars"; //must be relative for batch operations
    private static final String EVENT_URL = "me/calendars/%s/events"; //must be relative for batch operations
    ...
    public MicrosoftCalendarImporter(OkHttpClient client, ObjectMapper objectMapper, TransformerService transformerService, JobStore jobStore) {
        this("https://graph.microsoft.com", client, objectMapper, transformerService, jobStore);
    }

    public MicrosoftCalendarImporter(String baseUrl, OkHttpClient client, ObjectMapper objectMapper, TransformerService transformerService, JobStore jobStore) {
        this.client = client;
    ...
    public ImportResult importItem(String jobId, TokenAuthData authData, CalendarContainerResource data) {
        ...
        for (CalendarEventModel event : data.getEvents()) {
            String importedId = calendarMappings.getImportedId(event.getCalendarId());  // get the imported calendar id for the event from the mappings
            Map<String, Object> request = createRequestItem(event, requestId, String.format(EVENT_URL, importedId), problems);
            requestId++;
    ...
    private Map<String, Object> createRequestItem(Object item, int id, String url, List<String> problems) {
