...
public class MicrosoftTransferServiceProvider implements TransferServiceProvider {
  private static final String CONTACTS =
      "contacts"; // TODO we should standardize the naming scheme if it isn't already
  ...
  public MicrosoftTransferServiceProvider(
      OkHttpClient client, ObjectMapper mapper, TransformerService transformerService) {
    exporterCache.put(CONTACTS, new MicrosoftContactsExporter(client, mapper, transformerService));
    importerCache.put(
        CONTACTS, new MicrosoftContactsImporter("", client, mapper, transformerService));
  }
  ...
  public Exporter<?, ?> getExporter(String transferDataType) {
    return exporterCache.computeIfAbsent(
        transferDataType,
        v -> {
  ...
  public Importer<?, ?> getImporter(String transferDataType) {
    return importerCache.computeIfAbsent(
        transferDataType,
        v -> {
