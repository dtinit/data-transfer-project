...
public class DaybookPostsImporter implements Importer<TokensAndUrlAuthData, SocialActivityContainerResource> {
    ...
    public ImportResult importItem(UUID jobId, IdempotentImportExecutor executor, TokensAndUrlAuthData authData, SocialActivityContainerResource resource) throws Exception {
        ...
        monitor.debug(() -> String.format("Number of Posts: %d", resource.getCounts().get("activitiesCount")));
        ...
        for (SocialActivityModel activity : resource.getActivities()) {
            if (activity.getType() == SocialActivityType.NOTE || activity.getType() == SocialActivityType.POST) {
                executor.importAndSwallowIOExceptions(activity, currentActivity -> {
    ...
    private ItemImportResult<String> insertActivity(SocialActivityModel activity, TokensAndUrlAuthData authData) throws IOException {
        ...
        FormBody.Builder builder = new FormBody.Builder().add("type", "POSTS").add("exporter", exportService).add("content", content).add("title", title).add("location", location).add("published", published);
