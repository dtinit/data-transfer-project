...
public class KoofrClient {
    ...
    public boolean fileExists(String path) throws IOException, InvalidTokenException {
        String url;
        try {
            url = getUriBuilder().setPath(API_PATH_PREFIX + "/mounts/primary/files/info").setParameter("path", path).build().toString();
    ...
    public void ensureFolder(String parentPath, String name) throws IOException, InvalidTokenException {
        ...
        String url;
        try {
            url = getUriBuilder().setPath(API_PATH_PREFIX + "/mounts/primary/files/folder").setParameter("path", parentPath).build().toString();
        } catch (URISyntaxException e) {
            throw new IllegalStateException("Could not produce url.", e);
        }
        Request.Builder requestBuilder = getRequestBuilder(url);
    ...
    public void addDescription(String path, String description) throws IOException, InvalidTokenException {
        ...
        String url;
        try {
            url = getUriBuilder().setPath(API_PATH_PREFIX + "/mounts/primary/files/tags/add").setParameter("path", path).build().toString();
        } catch (URISyntaxException e) {
            throw new IllegalStateException("Could not produce url.", e);
        }
        Request.Builder requestBuilder = getRequestBuilder(url);
        requestBuilder.post(RequestBody.create(MediaType.parse("application/json"), objectMapper.writeValueAsString(body)));
        try (Response response = getResponse(requestBuilder)) {
            int code = response.code();
            if ((code < 200 || code > 299) && code != 409) {
                throw new KoofrClientIOException(response);
    ...
    public List<FilesListRecursiveItem> listRecursive(String path) throws IOException, InvalidTokenException {
        String url;
        try {
            URIBuilder builder = getUriBuilder().setPath(CONTENT_API_PATH_PREFIX + "/mounts/primary/files/listrecursive").setParameter("path", path);
            url = builder.build().toString();
        } catch (URISyntaxException e) {
            throw new IllegalStateException("Could not produce url.", e);
        }
        ...
        try (Response response = getResponse(requestBuilder)) {
            ...
            try (final Reader bodyReader = new InputStreamReader(body.byteStream(), StandardCharsets.UTF_8);
    ...
    public String fileLink(String path) throws IOException, InvalidTokenException {
        String url;
        try {
            url = getUriBuilder().setPath(API_PATH_PREFIX + "/mounts/primary/files/download").setParameter("path", path).build().toString();
        } catch (URISyntaxException e) {
            throw new IllegalStateException("Could not produce url.", e);
        }
        Request.Builder requestBuilder = getRequestBuilder(url);
