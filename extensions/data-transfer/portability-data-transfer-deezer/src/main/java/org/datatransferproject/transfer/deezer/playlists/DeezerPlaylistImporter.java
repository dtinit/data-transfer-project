...
public class DeezerPlaylistImporter
  ...
  public DeezerPlaylistImporter(
      Monitor monitor, HttpTransport httpTransport, TransferServiceConfig transferServiceConfig) {
  ...
  private void createPlaylist(
      IdempotentImportExecutor idempotentExecutor, DeezerApi api, MusicPlaylist playlist)
      throws Exception {
    Long newPlaylistId =
        idempotentExecutor.executeAndSwallowIOExceptions(
            playlist.getIdentifier(), playlist.getHeadline(), () -> createPlaylist(api, playlist));
        ...
        () -> {
          ...
          if (insertResponse != null) {
            throw new IOException(
                "problem inserting tracks into playlist: "
                    + playlist
                    + " error: "
                    + insertResponse);
          }
  ...
  private Long createPlaylist(DeezerApi api, MusicPlaylist playlist) {
    try {
      ...
      if (createResponse.getError() != null) {
        throw new IOException(
            "problem creating playlist: " + playlist + " error: " + createResponse.getError());
      }
      if (createResponse.getError() != null) {
        throw new IOException(
            "Problem creating playlist: " + playlist + ": " + createResponse.getError());
      }
