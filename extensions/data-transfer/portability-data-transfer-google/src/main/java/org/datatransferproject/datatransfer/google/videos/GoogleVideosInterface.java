...
public class GoogleVideosInterface {
    ...
    BatchMediaItemResponse createVideo(NewMediaItemUpload newMediaItemUpload) throws IOException {
        ...
        return makePostRequest(BASE_URL + "mediaItems:batchCreate", Optional.empty(), httpContent, BatchMediaItemResponse.class);
    }
    ...
    MediaItemSearchResponse listVideoItems(Optional<String> pageToken) throws IOException {
        ...
        params.put(FILTERS_KEY, ImmutableMap.of(MEDIA_FILTER_KEY, ImmutableMap.of("mediaTypes", ImmutableList.of("VIDEO"))));
        if (pageToken.isPresent()) {
            params.put(TOKEN_KEY, pageToken.get());
        }
        HttpContent content = new JsonHttpContent(this.jsonFactory, params);
        return makePostRequest(BASE_URL + "mediaItems:search", Optional.empty(), content, MediaItemSearchResponse.class);
    }
    ...
    <T> T makePostRequest(String url, Optional<Map<String, String>> parameters, HttpContent httpContent, Class<T> clazz) throws IOException {
        ...
        HttpRequest postRequest = requestFactory.buildPostRequest(new GenericUrl(url + "?" + generateParamsString(parameters)), httpContent);
    ...
    public static Pair<String, Long> uploadVideo(UUID jobId, VideoModel video, PhotosLibraryClient photosLibraryClient, TemporaryPerJobDataStore dataStore, ConnectionProvider connectionProvider) throws IOException, UploadErrorException, InvalidTokenException {
        final File tmp = createTempFile(jobId, dataStore, connectionProvider, video);
        try {
    ...
    private static File createTempFile(UUID jobId, TemporaryPerJobDataStore dataStore, ConnectionProvider connectionProvider, DownloadableFile video) throws IOException {
        ...
        return createTempFile(jobId, dataStore, connectionProvider, video, fileSuffix);
    }
        ...
        try {
            for (VideoModel video : batchedVideos) {
                try {
                    ...
                    final String googleAlbumId = Strings.isNullOrEmpty(video.getAlbumId()) ? null : executor.getCachedValue(video.getAlbumId());
                    ...
                    if (e instanceof FileNotFoundException) {
                        ...
                        monitor.info(() -> String.format("Video resource was missing for id: %s", video.getIdempotentId()), e);
                        continue;
                    }
                    executor.importAndSwallowIOExceptions(video, videoModel -> ItemImportResult.error(e, null));
                }
            }
            ...
            final List<NewMediaItemResult> resultsList = mediaItemsByAlbum.keySet().stream().map(k -> k == null ? client.batchCreateMediaItems(mediaItemsByAlbum.get(null)) : client.batchCreateMediaItems(k, mediaItemsByAlbum.get(k))).map(BatchCreateMediaItemsResponse::getNewMediaItemResultsList).flatMap(Collection::stream).collect(Collectors.toList());
            long bytes = 0L;
            for (NewMediaItemResult result : resultsList) {
                ...
                if (code == Code.OK_VALUE) {
                    executor.importAndSwallowIOExceptions(video, videoModel -> ItemImportResult.success(result.getMediaItem().getId(), length));
                    if (length != null) {
                        bytes += length;
                    }
                } else {
                    executor.importAndSwallowIOExceptions(video, videoModel -> ItemImportResult.error(new IOException(String.format("Video item could not be created. Code: %d Message: %s", code, result.getStatus().getMessage())), length));
                }
            ...
            if (!uploadTokenToDataId.isEmpty()) {
                for (Entry<String, VideoModel> entry : uploadTokenToDataId.entrySet()) {
                    ...
                    executor.importAndSwallowIOExceptions(video, videoModel -> ItemImportResult.error(new IOException("Video item was missing from results list."), uploadTokenToLength.get(uploadToken)));
                }
            }
            return bytes;
