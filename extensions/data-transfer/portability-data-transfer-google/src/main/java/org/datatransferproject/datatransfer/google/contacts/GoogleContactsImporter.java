...
public class GoogleContactsImporter implements Importer<TokensAndUrlAuthData, ContactsModelWrapper> {
    ...
    private static com.google.api.services.people.v1.model.Address convertToGoogleAddress(ezvcard.property.Address vCardAddress) {
        ...
        personAddress.setMetadata(vCardAddress.getPref() != null && vCardAddress.getPref() == VCARD_PRIMARY_PREF ? PRIMARY_FIELD_METADATA : SECONDARY_FIELD_METADATA);
    ...
    private synchronized PeopleService makePeopleService(TokensAndUrlAuthData authData) {
        ...
        return new PeopleService.Builder(credentialFactory.getHttpTransport(), credentialFactory.getJsonFactory(), credential).setApplicationName(GoogleStaticObjects.APP_NAME).build();
    }
}
