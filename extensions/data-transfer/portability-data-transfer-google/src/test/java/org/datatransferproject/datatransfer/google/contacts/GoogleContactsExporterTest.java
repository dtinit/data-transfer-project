...
public class GoogleContactsExporterTest {
    ...
    private static final FieldMetadata PRIMARY_FIELD_METADATA = new FieldMetadata().setSource(SOURCE).setPrimary(true);

    private static final Name NAME = new Name().setFamilyName("Turing").setGivenName("Alan").setMetadata(PRIMARY_FIELD_METADATA);

    private static final Person PERSON = new Person().setNames(Collections.singletonList(NAME)).setResourceName(RESOURCE_NAME);
