...
public class GoogleMediaExporterTest {
    ...
    public void setup() throws IOException, InvalidTokenException, PermissionDeniedException, UploadErrorException {
        ...
        googleMediaExporter = new GoogleMediaExporter(credentialFactory, jobStore, GsonFactory.getDefaultInstance(), photosInterface, monitor);
    ...
    public void exportAlbumFirstSet() throws IOException, InvalidTokenException, PermissionDeniedException {
        ...
        // Run test
        ExportResult<MediaContainerResource> result = googleMediaExporter.exportAlbums(null, /*authData*/
        Optional.empty(), uuid);
        // Check results
        ...
        assertThat(actualAlbums.stream().map(MediaAlbum::getId).collect(Collectors.toList())).containsExactly(ALBUM_ID);
    ...
    public void exportPhotoFirstSet() throws IOException, InvalidTokenException, PermissionDeniedException, UploadErrorException {
        ...
        ExportResult<MediaContainerResource> result = googleMediaExporter.exportMedia(null, Optional.of(idOnlyContainerResource), Optional.empty(), uuid);
        // Check results
        // Verify correct methods were called
        ...
        "some://fake/gphotoapi/uri=d");
        assertThat(actualPhotos.stream().map(PhotoModel::getAlbumId).collect(Collectors.toList())).containsExactly(ALBUM_ID);
        assertThat(actualPhotos.stream().map(PhotoModel::getName).collect(Collectors.toList())).containsExactly(FILENAME);
    }

    @Test
    public void exportVideoFirstSet() throws IOException, InvalidTokenException, PermissionDeniedException, UploadErrorException {
        setUpSingleAlbum();
        when(albumListResponse.getNextPageToken()).thenReturn(null);
        GoogleMediaItem mediaItem = setUpSingleVideo("some://fake/gphotoapi/uri", "some-upstream-generated-video-id");
        when(mediaItemSearchResponse.getMediaItems()).thenReturn(new GoogleMediaItem[] { mediaItem });
        when(mediaItemSearchResponse.getNextPageToken()).thenReturn(MEDIA_TOKEN);
        IdOnlyContainerResource idOnlyContainerResource = new IdOnlyContainerResource(ALBUM_ID);
        ExportResult<MediaContainerResource> result = googleMediaExporter.exportMedia(null, Optional.of(idOnlyContainerResource), Optional.empty(), uuid);
        ...
        assertThat(actualVideos.stream().map(VideoModel::getFetchableUrl).collect(Collectors.toList())).containsExactly(// for download
        "some://fake/gphotoapi/uri=dv");
        assertThat(actualVideos.stream().map(VideoModel::getAlbumId).collect(Collectors.toList())).containsExactly(ALBUM_ID);
        assertThat(actualVideos.stream().map(VideoModel::getName).collect(Collectors.toList())).containsExactly(FILENAME);
    }
    ...
    public void exportPhotoSubsequentSet() throws IOException, InvalidTokenException, PermissionDeniedException, UploadErrorException {
        ...
        StringPaginationToken inputPaginationToken = new StringPaginationToken(MEDIA_TOKEN_PREFIX + MEDIA_TOKEN);
        IdOnlyContainerResource idOnlyContainerResource = new IdOnlyContainerResource(ALBUM_ID);
        // Run test
        ExportResult<MediaContainerResource> result = googleMediaExporter.exportMedia(null, Optional.of(idOnlyContainerResource), Optional.of(inputPaginationToken), uuid);
        // Check results
        // Verify correct methods were called
    ...
    @Test
    public void populateContainedMediaList() throws IOException, InvalidTokenException, PermissionDeniedException, UploadErrorException {
    ...
    @Test
    public /* Tests that when there is no album information passed along to exportMedia, only albumless
  photos are exported.
  */
    void onlyExportAlbumlessPhoto() throws IOException, InvalidTokenException, PermissionDeniedException, UploadErrorException {
        ...
        // Run test
        ExportResult<MediaContainerResource> result = googleMediaExporter.exportMedia(null, Optional.empty(), Optional.empty(), uuid);
        // Check results
    ...
    private static GoogleMediaItem setUpSinglePhoto(String imageUri, String photoId) {
        ...
        GoogleMediaItem googleMediaItem = setUpSingleMediaItem(imageUri, photoId, mediaMetadata);
    ...
    private static GoogleMediaItem setUpSingleVideo(String videoUri, String videoId) {
        ...
        GoogleMediaItem googleMediaItem = setUpSingleMediaItem(videoUri, videoId, mediaMetadata);
