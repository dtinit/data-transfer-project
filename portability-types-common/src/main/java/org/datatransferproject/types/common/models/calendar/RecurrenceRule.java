...
public class RecurrenceRule {
    ...
    static final List<String> byRuleList = Arrays.stream(ByRule.values()).map(Object::toString).collect(Collectors.toList());

    static final List<String> dayList = Arrays.stream(Day.values()).map(Object::toString).collect(Collectors.toList());
    ...
    public static RRule parseRRuleString(String rRuleString) {
        ...
        for (String property : components) {
            ...
            if (key.equals(FREQ)) {
                ...
                // TODO(olsona): consider parsing further?
                builder.until(value);
    ...
    enum TimeType {
        ...
        DATETIME, DATE, PERIOD, UNDEFINED
    }

    @AutoValue
    public abstract static class RRule {

        static RRule create(Freq freq, String until, int count, int interval, Map<ByRule, String> byRuleMap, Day wkst) {
            return builder().freq(freq).until(until).count(count).interval(interval).byRuleMap(byRuleMap).wkst(wkst).build();
        }
        ...
        @Nullable
        abstract String // represents a date/time
        until();
        ...
        @Nullable
        abstract Map<ByRule, String> // TODO(olsona); parse properties further
        byRuleMap();
