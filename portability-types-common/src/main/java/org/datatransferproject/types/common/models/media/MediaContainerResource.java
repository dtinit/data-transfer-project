...
public class MediaContainerResource extends ContainerResource {
    ...
    @JsonCreator
    public MediaContainerResource(@JsonProperty("albums") Collection<MediaAlbum> albums, @JsonProperty("photos") Collection<PhotoModel> photos, @JsonProperty("videos") Collection<VideoModel> videos) {
    ...
    public static MediaContainerResource photoToMedia(PhotosContainerResource photosContainer) {
        return new MediaContainerResource(photosContainer.getAlbums().stream().map(MediaAlbum::photoToMediaAlbum).collect(Collectors.toList()), photosContainer.getPhotos(), null);
    }
    ...
    public static PhotosContainerResource mediaToPhoto(MediaContainerResource mediaContainer) {
        return new PhotosContainerResource(mediaContainer.getAlbums().stream().map(MediaAlbum::mediaToPhotoAlbum).collect(Collectors.toList()), mediaContainer.getPhotos());
    }
    ...
    public static MediaContainerResource videoToMedia(VideosContainerResource videosContainer) {
        return new MediaContainerResource(videosContainer.getAlbums().stream().map(MediaAlbum::videoToMediaAlbum).collect(Collectors.toList()), null, /*photos*/
    ...
    public static VideosContainerResource mediaToVideo(MediaContainerResource mediaContainer) {
        return new VideosContainerResource(mediaContainer.getAlbums().stream().map(MediaAlbum::mediaToVideoAlbum).collect(Collectors.toList()), mediaContainer.getVideos());
    }
    ...
    public Map<String, Integer> getCounts() {
        return new ImmutableMap.Builder<String, Integer>().put(ALBUMS_COUNT_DATA_NAME, albums.size()).put(PHOTOS_COUNT_DATA_NAME, photos.size()).put(VIDEOS_COUNT_DATA_NAME, videos.size()).build();
    }
    ...
    public boolean equals(Object o) {
        ...
        return Objects.equals(getAlbums(), that.getAlbums()) && Objects.equals(getPhotos(), that.getPhotos()) && Objects.equals(getVideos(), that.getVideos());
    }
    ...
    private void transmogrifyTitles(TransmogrificationConfig config) {
        ...
        for (PhotoModel photo : photos) {
            photo.cleanTitle(config.getPhotoTitleForbiddenCharacters(), config.getPhotoTitleReplacementCharacter(), config.getPhotoTitleMaxLength());
        }
        for (VideoModel video : videos) {
            video.cleanName(config.getVideoTitleForbiddenCharacters(), config.getVideoTitleReplacementCharacter(), config.getVideoTitleMaxLength());
        }
        for (MediaAlbum album : albums) {
            album.cleanName(config.getAlbumNameForbiddenCharacters(), config.getAlbumNameReplacementCharacter(), config.getAlbumNameMaxLength());
        }
    }
    ...
    public String toString() {
        return MoreObjects.toStringHelper(this).add("albums", getAlbums()).add("photos", getPhotos()).add("videos", getVideos()).add("counts", getCounts()).toString();
    }
