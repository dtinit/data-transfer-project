...
public class RetryingCallable<T> implements Callable<T> {
    ...
    public RetryingCallable(Callable<T> callable, RetryStrategyLibrary retryStrategyLibrary, Clock clock, Monitor monitor, DataVertical dataType, String service) {
    ...
    public RetryingCallable(Callable<T> callable, RetryStrategyLibrary retryStrategyLibrary, Clock clock, Monitor monitor) {
    ...
    public T call() throws RetryException {
        while (true) {
            ...
            attempts++;
            try {
                monitor.debug(() -> String.format("Attempt %d started, service: %s, dataType: %s", attempts, service, dataType));
                ...
                monitor.debug(() -> String.format("Attempt %d failed, using retry strategy: %s, service: %s, dataType: %s", attempts, strategy.toString(), service, dataType));
                if (strategy.canTryAgain(attempts)) {
                    ...
                    monitor.debug(() -> String.format("Strategy has %d remainingIntervalMillis after %d elapsedMillis", nextAttemptIntervalMillis, elapsedMillis));
                    ...
                    monitor.debug(() -> String.format("Strategy canTryAgain returned false after %d retries", attempts));
                    throw new RetryException(attempts, mostRecentException);
                }
            }
        }
