...
import org.dataportabilityproject.gateway.crypto.DecrypterFactory;
...
final class Oauth2CallbackHandler implements HttpHandler {
  ...
  private String handleExchange(HttpExchange exchange) throws IOException {
    ...
    try {
      ...
      String jobId = ReferenceApiUtils.decodeId(encodedIdCookie.getValue());
      String state = ReferenceApiUtils.decodeId(authResponse.getState());
      ...
      String encryptedInitialAuthData =
          (transferMode == TransferMode.EXPORT) ? job.getJobAuthorization()
              .getInitialExportAuthData() : job.getJobAuthorization().getInitialImportAuthData();
      ...
      AuthData authData = generator
          .generateAuthData(ApiFlags.baseApiUrl(), authResponse.getCode(), jobId,
