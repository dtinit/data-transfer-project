...
public class JobPollingServiceTest {
    ...
    public void pollingLifeCycle() throws Exception {
        ...
        job = PortabilityJob.builder().setTransferDataType(DataVertical.PHOTOS).setExportService("DummyExportService").setImportService("DummyImportService").setAndValidateJobAuthorization(JobAuthorization.builder().setEncryptionScheme("cleartext").setState(State.INITIAL).setSessionSecretKey("fooBar").build()).build();
