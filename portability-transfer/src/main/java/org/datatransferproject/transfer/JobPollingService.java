...
class JobPollingService extends AbstractScheduledService {
    ...
    @Inject
    JobPollingService(JobStore store, TransferKeyGenerator transferKeyGenerator, PublicKeySerializer publicKeySerializer, Scheduler scheduler, Monitor monitor, ExtensionContext context) {
    ...
    protected void runOneIteration() {
        ...
        if (JobMetadata.isInitialized()) {
            if (stopwatch.elapsed(TimeUnit.SECONDS) > credsTimeoutSeconds) {
                ...
                String message = format("Waited over %d seconds for the creds to be provided on the claimed job: %s", credsTimeoutSeconds, jobId);
    ...
    private void markJobTimedOut(UUID jobId) {
        try {
            ...
            monitor.severe(() -> format("IOException while marking job as timed out. JobId: %s", jobId), e);
        }
    }
    ...
    private void pollForUnassignedJob() {
        ...
        if (claimed) {
            monitor.debug(() -> format("Updated job %s to CREDS_ENCRYPTION_KEY_GENERATED, publicKey length: %s", jobId, keyPair.getEncodedPublicKey().length));
            stopwatch.start();
        }
    }
