...
class JobCancelWatchingService extends AbstractScheduledService {
    ...
    @Inject
    JobCancelWatchingService(JobStore store, @Annotations.CancelScheduler Scheduler scheduler, Monitor monitor, DtpInternalMetricRecorder dtpInternalMetricRecorder) {
        this.store = store;
    ...
    private void recordGeneralMetric(String jobState) {
        dtpInternalMetricRecorder.recordGenericMetric(JobMetadata.getDataType(), JobMetadata.getExportService(), jobState, JobMetadata.getStopWatch().elapsed());
    }
}
