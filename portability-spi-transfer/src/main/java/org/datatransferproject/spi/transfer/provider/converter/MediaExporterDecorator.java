...
public class MediaExporterDecorator<AD extends AuthData> implements Exporter<AD, MediaContainerResource> {
    ...
    public MediaExporterDecorator(Exporter<AD, PhotosContainerResource> photosExporter, Exporter<AD, VideosContainerResource> videosExporter) {
        this.photosExporter = photosExporter;
        this.videosExporter = videosExporter;
    }

    @Override
    public ExportResult<MediaContainerResource> export(UUID jobId, AD authData, Optional<ExportInformation> exportInfo) throws Exception {
    ...
    private ExportResult<MediaContainerResource> mergeResults(ExportResult<PhotosContainerResource> photoResult, ExportResult<VideosContainerResource> videoResult) {
        ResultType resultType = ResultType.merge(photoResult.getType(), videoResult.getType());
        Collection<MediaAlbum> albums = mergeAlbums(photoResult.getExportedData().getAlbums(), videoResult.getExportedData().getAlbums());
        MediaContainerResource exportData = new MediaContainerResource(albums, photoResult.getExportedData().getPhotos(), videoResult.getExportedData().getVideos());
        ContinuationData cd = mergeContinuationData(photoResult.getContinuationData(), videoResult.getContinuationData());
        return new ExportResult<>(resultType, exportData, cd);
    }

    private ExportResult<VideosContainerResource> exportVideos(UUID jobId, AD authData, Optional<ExportInformation> exportInfo) throws Exception {
    ...
    private ExportResult<PhotosContainerResource> exportPhotos(UUID jobId, AD authData, Optional<ExportInformation> exportInfo) throws Exception {
    ...
    private Collection<MediaAlbum> mergeAlbums(Collection<PhotoAlbum> a1, Collection<VideoAlbum> a2) {
        return Stream.concat(a1.stream().map(MediaAlbum::photoToMediaAlbum), a2.stream().map(MediaAlbum::videoToMediaAlbum)).distinct().collect(Collectors.toList());
    }
}
