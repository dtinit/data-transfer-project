...
public class AnyToAnyExporter<AD extends AuthData, From extends ContainerResource, To extends ContainerResource> implements Exporter<AD, To> {
    ...
    public ExportResult<To> export(UUID jobId, AD authData, Optional<ExportInformation> exportInfo) throws Exception {
        Optional<ExportInformation> infoWithConvertedResource = exportInfo.map((ei) -> ei.copyWithResource(exportInformationConverter.apply(ei.getContainerResource())));
        ExportResult<From> originalResult = exporter.export(jobId, authData, infoWithConvertedResource);
        return originalResult.copyWithExportedData(containerResourceConverter.apply(originalResult.getExportedData()));
    }
}
