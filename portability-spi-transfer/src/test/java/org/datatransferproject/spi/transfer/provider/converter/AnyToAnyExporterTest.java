...
class AnyToAnyExporterTest {
    ...
    @Test
    public void shouldHandleConversionFromPhotoExporterToMediaExporterUsingAnyInputContainerResource() throws Exception {
        Exporter<AuthData, PhotosContainerResource> photosExporter = (jobId, authData, exportInformation) -> new ExportResult<>(ResultType.END, exportInformation.map(ei -> (PhotosContainerResource) ei.getContainerResource()).get());
        AnyToAnyExporter<AuthData, PhotosContainerResource, MediaContainerResource> mediaExporter = new AnyToAnyExporter<>(photosExporter, MediaContainerResource::photoToMedia, (cr) -> {
