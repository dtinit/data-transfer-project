...
public class MediaExporterDecoratorTest {
    ...
    @BeforeEach
    public void setUp() {
        photosExporter = (jobId, authData, exportInformation) -> new ExportResult<>(ResultType.END, exportInformation.map(ei -> (PhotosContainerResource) ei.getContainerResource()).orElse(new PhotosContainerResource(null, null)));
        videosExporter = (jobId, authData, exportInformation) -> new ExportResult<>(ResultType.END, exportInformation.map(ei -> (VideosContainerResource) ei.getContainerResource()).orElse(new VideosContainerResource(null, null)));
        mediaExporter = new MediaExporterDecorator<>(photosExporter, videosExporter);
        albums = List.of(new MediaAlbum("album1", "name1", "desc1"), new MediaAlbum("album2", "name2", "desc2"), new MediaAlbum("album3", "name3", "desc3"));
        photos = List.of(new PhotoModel("p1", "", null, null, "d1", null, false, null, null), new PhotoModel("p2", "", null, null, "d2", "a1", false, null, null), new PhotoModel("p3", "", null, null, "d3", "a2", false, null, null));
        videos = List.of(new VideoModel("v1", "", null, null, "d4", null, false, null), new VideoModel("v2", "", null, null, "d5", "a3", false, null), new VideoModel("v3", "", null, null, "d6", null, false, null));
    }
    ...
    public void shouldHandleEmptyExportInfomationAndEmptyResults() throws Exception {
        ...
        ExportResult<MediaContainerResource> exp = new ExportResult<>(ResultType.END, new MediaContainerResource(null, null, null));
    ...
    public void shouldHandleErrorInPhotos() throws Exception {
        ...
        mediaExporter = new MediaExporterDecorator<>((id, ad, ei) -> new ExportResult<>(throwable), videosExporter);
        MediaContainerResource mcr = new MediaContainerResource(albums, photos, videos);
        Optional<ExportInformation> ei = Optional.of(new ExportInformation(null, mcr));
        ExportResult<MediaContainerResource> res = mediaExporter.export(null, null, ei);
    ...
    public void shouldHandleErrorInVideos() throws Exception {
        Exception throwable = new Exception();
        mediaExporter = new MediaExporterDecorator<>(photosExporter, (id, ad, ei) -> new ExportResult<>(throwable));
        MediaContainerResource mcr = new MediaContainerResource(albums, photos, videos);
        Optional<ExportInformation> ei = Optional.of(new ExportInformation(null, mcr));
        ExportResult<MediaContainerResource> res = mediaExporter.export(null, null, ei);
        assertEquals(new ExportResult<>(throwable), res);
