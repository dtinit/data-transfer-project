...
export class TransferService {
    ...
    createTransferJob(createTransferJob: CreateTransferJob): Observable<TransferJob> {
        return this.http.post(`${environment.apiUrl}/transfer`, createTransferJob) as Observable<TransferJob>;
    }
    ...
    reserveWorker(reserveWorker: ReserveWorker): Observable<string> {
        return this.http.post(`${environment.apiUrl}/transfer/worker/${reserveWorker.id}`, reserveWorker) as Observable<string>;
    }

    getReservedWorker(getWorker: GetReservedWorker): Observable<ReservedWorker> {
        return this.http.get(`${environment.apiUrl}/transfer/worker/${getWorker.id}`) as Observable<ReservedWorker>;
    }

    startTransferJob(start: StartTransferJob): Observable<TransferJob> {
        return this.http.post(`${environment.apiUrl}/transfer/${start.id}/start`, start) as Observable<TransferJob>;
    }
}
